...
export default class ArticleEditor extends Component {
    ...
    getContentDetails(id, libconCode) {
        ...
        fetch(`${process.env.PATH_URL}showcontent?libid=${libconCode}&id=${id}`, {
            method: "GET",
            headers: {
    ...
    checkSaveContent() {
        if (this.state.heading !== "" && this.state.order !== "") {
            ...
            console.log("heading :- ", this.state.heading, ", editor :- ", this.state.editorState, " order :- ", this.state.order)
        }
    }

    saveContent() {

        const { editorState, heading, order, system, app, profileImg, contentId, libconCode } = this.state;
        fetch(`${process.env.PATH_URL}savecontent`, {
            method: "POST",
            headers: {
